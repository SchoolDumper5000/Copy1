...
class AccManageInterface:
  ...
  def change_name(self):
    while True:
        os.system("clear")
        self.ui.default_message(f"Ｃｕｒｒｅｎｔ ｎａｍｅ ｓｅｔ ｔｏ：'{self.creds[4]}'\nＴｏ ｇｏ ｂａｃｋ ｐｒｅｓｓ ［ ＥＮＴＥＲ ］")
        ...
        else:self.ui.error_message("Ｓｔａｔｕｓ： Ｆａｉｌｅｄ ｔｏ ｕｐｄａｔｅ ｎａｍｅ")
        time.sleep(1.5)
        break
    ...
    while True:
      ...
      if res:
        ...
        if new_password == confirm_password:
          self.auth.update_password(self.logged_in_user, old_password,new_password)
          self.ui.success_message("Ｓｔａｔｕｓ： ｐａｓｓｗｏｒｄ ｕｐｄａｔｅｄ ｓｕｃｃｅｓｓｆｕｌｌｙ！")
          break
        else:
          self.ui.error_message("Ｓｔａｔｕｓ： Ｅｎｔｅｒｅｄ ｐａｓｓｗｏｒｄｓ ｄｏ ｎｏｔ ｍａｔｃｈ")
          break
      else:self.ui.error_message("Ｓｔａｔｕｓ： Ｐａｓｓｗｏｒｄ ｄｏｅｓ ｎｏｔ ｍａｔｃｈ") 
  def delete_account(self):
    os.system("clear")
    options = ["Ｙｅｓ\n", "Ｎｏ\n"]
    index = survey.routines.select('\nＳＥＬＥＣＴ Ａ ＯＰＥＲＡＴＩＯＮ:\n',options=options,focus_mark='>',
                                   evade_color=survey.colors.basic('white'),insearch_color=survey.colors.basic('white'))
    if index == 0:
      os.system("clear")
    ...
    else:return ("MANAGE_ACCOUNT", self.logged_in_user)
...
""")
    ...
    """)
    self.ui.primary_line("grey", 100)
    options = ["Ｃｈａｎｇｅ Ｐａｓｓｗｏｒｄ\n", "Ｃｈａｎｇｅ Ｎａｍｅ\n", "Ｄｅｌｅｔｅ Ａｃｃｏｕｎｔ\n", "Ｇｏ Ｂａｃｋ\n"]
    index = survey.routines.select('\nＳＥＬＥＣＴ Ａ ＯＰＥＲＡＴＩＯＮ:\n',options=options,focus_mark='>',
                                   evade_color=survey.colors.basic('white'),insearch_color=survey.colors.basic('white'))
    if index == 3:return ("MENU", self.logged_in_user)
    elif index == 0:
